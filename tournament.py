...
def load_data():
    ...
    if r.status_code == 200:
        ...
        print(f"GitHub load error: {r.status_code} {r.text}")
        return DEFAULT_DATA.copy(), None

def save_data(data, sha=None):
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/{GITHUB_FILE_PATH}"
    encoded_content = base64.b64encode(json.dumps(data, indent=4).encode()).decode()
    payload = {"message": "Update tournament data", "content": encoded_content}
    if sha:
        payload["sha"] = sha
...
def setup_tournament_commands(tree: app_commands.CommandTree, allowed_role_ids):
    ...
    async def start_match(interaction: discord.Interaction, a_item, b_item):
        embed = discord.Embed(title="Vote for the winner!", description=f"🇦 {a_item}\n🇧 {b_item}")
    ...
    async def next_match(interaction: discord.Interaction):
        ...
        # If current round empty → move next_round to current_round
        if not data["current_round"]:
            if len(data["next_round"]) > 1:
                ...
                final_winner = data["next_round"][0]
    ...
    async def startwc(interaction: discord.Interaction, title: str):
        ...
        if len(data["items"]) % 2 != 0:
            await interaction.response.send_message("❌ Must have an even number of items to start!", ephemeral=True)
            return
    ...
    @tree.command(name="addwcitem", description="Add one or multiple items (comma separated)")
    ...
    @tree.command(name="scoreboard", description="View the scoreboard")
