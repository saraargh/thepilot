...
async def count_votes_from_message(guild, channel_id, message_id):
    """
    Returns (a_count, b_count) for VOTE_A / VOTE_B.
    Unique users only; last vote counts.
    """
    try:
    ...
    for reaction in msg.reactions:
        ...
        for u in users:
            if u.bot:
                continue
            user_last_choice[u.id] = emoji  # last reaction wins

    a_count = sum(1 for v in user_last_choice.values() if v == VOTE_A)
    b_count = sum(1 for v in user_last_choice.values() if v == VOTE_B)
...
def setup_tournament_commands(tree: app_commands.CommandTree, allowed_role_ids):
    ...
    async def post_next_match(channel, data, sha):
        ...
        stores last_match in data, and continuously updates live vote lists.
        """
        ...
        data["last_match"] = {"a": a, "b": b, "message_id": None, "channel_id": channel.id, "voters": {}}
        ...
        # Custom announcement
        await channel.send(f"@everyone, the next world cup of {data.get('title','items')} fixture is upon us! 🗳️")
            ...
            while True:
                try:
                    ...
                    for reaction in msg.reactions:
                        ...
                        for u in users:
                            # last vote counts
                            data["last_match"]["voters"][u.id] = emoji
                    voters_dict = data["last_match"]["voters"]
                    for uid, choice in voters_dict.items():
                        member = channel.guild.get_member(uid)
                        if not member:
                            continue
                        if choice == VOTE_A:
                            votes_a.append(member.display_name)
                        else:
                            votes_b.append(member.display_name)
                    desc = f"{VOTE_A} {a} — {', '.join(votes_a) if votes_a else 'No votes yet'}\n"
                    desc += f"{VOTE_B} {b} — {', '.join(votes_b) if votes_b else 'No votes yet'}"
        ...
        asyncio.create_task(update_votes())
        return sha
    ...
    async def removewcitem(interaction: discord.Interaction, items: str):
        ...
        if removed:
            ...
            await interaction.response.send_message("⚠️ No items removed (not found).", ephemeral=False)
    ...
    async def nextwcround(interaction: discord.Interaction):
        ...
        # Count votes from stored voters
        a_votes = sum(1 for v in lm["voters"].values() if v == VOTE_A)
        b_votes = sum(1 for v in lm["voters"].values() if v == VOTE_B)
        a_item = lm["a"]
        b_item = lm["b"]
        ...
        if len(data["current_round"]) >= 2:
            ...
            await interaction.channel.send("@everyone The round is complete!")
            await interaction.channel.send(embed=embed)
    ...
    # ---------------- /wchelp ----------------
    @tree.command(name="wchelp", description="List all World Cup commands")
    async def wchelp(interaction: discord.Interaction):
        help_text = (
            ...
            "/nextwcround — Count votes and advance matches\n"
            "/wcscoreboard — Show current scoreboard\n"
            "/showwcmatchups — Show current and upcoming matchups\n"
            "/resetwc — Reset World Cup data\n"
            "/endwc — End the World Cup\n"
        )
        embed = discord.Embed(title="📝 World Cup Commands", description=help_text, color=discord.Color.orange())
