...
def setup_tournament_commands(tree: app_commands.CommandTree, allowed_role_ids):
        ...
        try:
            while True:
                reaction, user = await interaction.client.wait_for(
                    "reaction_add", timeout=86400, check=check
                )
    ...
    async def startwc(interaction: discord.Interaction, title: str):
        ...
        save_data(data, sha)
        await interaction.response.send_message(f"🏁 Starting **{data['title']}**! Use `/nextwcround` to run each match.")
    ...
    async def nextwcround(interaction: discord.Interaction):
        ...
        if not data["running"] or not data["current_round"]:
            await interaction.response.send_message("❌ No active tournament or no matches left.", ephemeral=True)
            return

        # Announce previous winner
        if data["last_winner"]:
            await interaction.response.send_message(f"🏆 Last matchup winner: **{data['last_winner']}**!")

        items = data["current_round"]
        if len(items) == 1:
            await interaction.channel.send(f"🎉 **{items[0]}** wins the **{data['title']}**!")
            data["running"] = False
            save_data(data, sha)
            return
        ...
        # Get next matchup
        a_item = items.pop(0)
        b_item = items.pop(0) if items else None
        ...
        if b_item:
            ...
            winner = a_item
            await interaction.channel.send(f"{a_item} automatically advances due to odd number of items.")

        # Track winner for next round
        data["next_round"].append(winner)
        data["scores"][winner] += 1
        data["last_winner"] = winner
        if not items:  # Round finished
